...
--  Precondition commented out because full extend of declrations not yet known
   ...
   function Do_Aggregate_Literal_Record (N : Node_Id) return Irep is
      N_Type_Raw        : constant Entity_Id := Etype (N);
      N_Type : constant Entity_Id := Non_Private_Type (Etype (N));
      N_Underlying_Type : constant Node_Id :=
        (if Ekind (N_Type_Raw) in Incomplete_Or_Private_Kind then
              N_Type
         else
            Etype (N_Type_Raw));
   begin
...
--              "Attribute applied to string is unsupported");
   ...
   function Do_Constant (N : Node_Id) return Irep is
      ...
      Is_Integer : constant Boolean := Is_Integer_Type (Etype (N));
