...
--  Precondition commented out because full extend of declrations not yet known
   ...
   is
      ...
      begin
         Put_Line_Diag ("Handle parameter Irep: " &
                          Irep_Kind'Image (Kind (Mem)));

         if Kind (Mem) /= I_Symbol_Type then
            Put_Line_Diag ("Not an I_Symbol_Type");
         else
            Put_Line_Diag ("An I_Symbol_Type");
         end if;

         if Kind (Followed_Type_Symbol) = I_C_Enum_Type then

            Put_Line_Diag ("Is an enumeration");
            declare
